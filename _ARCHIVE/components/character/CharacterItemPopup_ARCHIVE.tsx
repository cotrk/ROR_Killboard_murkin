    ...
    <div className="popup">
        ...
        <div key={talisman.id} className="is-flex is-flex-direction-row my-2">
          ...
          <div>
            ...
            {talisman.stats.map((stat, index) => (
              <div key={`talisman-${talisman.id}-stat-${index}`} className="is-size-7 stats-text-highlight">
                + {stat.value * statMultiplier(stat.stat)}
                {isPercentage(stat.stat)} {t(`enums:stat.${stat.stat}`)}
              </div>
            ))}
      ...
      {item.itemSet && (
        <div className="is-size-7 mb-3 item-text-set-bonus-enabled">
          {item.itemSet.name}
          {item.itemSet.bonuses.map((bonus, index) => (
            <div
              key={`set-bonus-${item.itemSet?.id}-${index}`}
              className={clsx('ml-2', {
      ...
      {item.buffs.length > 0 &&
        item.buffs.map((buff, index) => (
          <div key={`item-buff-${item.id}-${index}`} className="is-size-7 item-text-buff">
            + {buff.description}
          </div>
        ))}
